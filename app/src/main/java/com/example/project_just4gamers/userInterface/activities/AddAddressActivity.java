...
public class AddAddressActivity extends AppCompatActivity {
    ...
    private void saveAddressToFirestore(){
        ...
        if (validate()){
            ...
            Address addressModel = new Address(new CloudFirestoreManager().getCurrentUserID(),
               ...
               if (addressDetails != null && !addressDetails.getId().isEmpty()){
                   new CloudFirestoreManager().updateAddress(AddAddressActivity.this, addressModel, addressDetails.getId());
               } else {
                   new CloudFirestoreManager().addAddress(AddAddressActivity.this, addressModel);
               }
        }
    }
    ...
    private boolean validate() {
        ...
        if (TextUtils.isEmpty(fullName)) {
            tietFullName.setError("Introdu numele tau.");
            return false;
        }

        if (TextUtils.isEmpty(phoneNumber) || phoneNumber.length() != 10) {
            tietPhoneNumber.setError("Trebuie sa introduci 10 caractere.");
            return false;
        }

        if (TextUtils.isEmpty(zipCode)){
            tietZipCode.setError("Introdu codul zip la adresei tale.");
            return false;
        }

        if (TextUtils.isEmpty(address)){
            tietZipCode.setError("Introdu adresa.");
            return false;
        }
        return true;
    }
    ...
    public void successAddAddress() {
        ...
        Toast.makeText(getApplicationContext(), "Adresa a fost adaugata cu succes!", Toast.LENGTH_SHORT).show();
        finish();
    }
