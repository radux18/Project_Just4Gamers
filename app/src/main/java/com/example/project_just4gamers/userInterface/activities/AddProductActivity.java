...
public class AddProductActivity extends AppCompatActivity {
    ...
    private boolean validate() {
        ...
        if (TextUtils.isEmpty(title)) {
            tiet_title.setError("Titlul este necesar.");
            return false;
        }
        if (TextUtils.isEmpty(price) || Float.parseFloat(price) <= 0) {
            tiet_price.setError("Pretul este necesar.");
            return false;
        }

        if (TextUtils.isEmpty(quantity) || Integer.parseInt(quantity) <= 0 ) {
            tiet_quantity.setError("Cantitate gresita.");
            return false;
        }
        if (TextUtils.isEmpty(description)) {
            tiet_description.setError("Este necesara o descriere.");
            return false;
        }
        return true;
    }
    ...
    private void uploadProductDetails(){
        ...
        String userName = this.getSharedPreferences(Constants.getPREFS(), Context.MODE_PRIVATE).getString(Constants.getUSERNAME(),"");
            ...
            if (type.equals(Constants.getNEW())){
                    ...
                    new CloudFirestoreManager().uploadProductDetails(AddProductActivity.this, product);
